...
def _load_prior_summaries():
    """Return {normalized_name: summary} from any existing cache file.

    Summaries are typically created by users (Supabase / manual edits) and are
    not regenerated by the scrape pipeline, so we must merge them back in on
    every full rebuild or they'd be lost. Checks both the canonical
    cache/card_data.json and the legacy root-level cards_cache.json.
    """
    prior = {}
    candidates = [
        os.path.join(CACHE_DIR, 'card_data.json'),
        os.path.join(HERE, 'cards_cache.json'),
    ]
    for path in candidates:
        ...
        for c in data:
            ...
            if name and summary:
                ...
                # First file wins (canonical cache > legacy cache).
                prior.setdefault(key, summary)
    return prior
...
def main():
    ...
    # Pull summaries forward from any prior cache so user-created summaries
    # survive a full rebuild. Manual overlays are applied later and still win.
    prior_summaries = _load_prior_summaries()
    if prior_summaries:
        print(f"      Preserving {len(prior_summaries)} summaries from prior cache")
    ...
    for i, kw in enumerate(keywords, 1):
        ...
        summary = kw.get("summary") or prior_summaries.get(
