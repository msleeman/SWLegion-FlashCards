"""
Large static data tables for SWLegion-FlashCards.
No functions — just data moved verbatim from build_swlegion_v4.py.
"""
import json
import os
...
# ── Ordered list of keyword pages on legion.takras.net ───────────────────────
KEYWORD_PAGES = [
...
# ── Keyword name -> CDN unit card image filename (loaded from data/keyword_images.json) ──
with open(os.path.join(_HERE, 'data', 'keyword_images.json'), encoding='utf-8') as _f:
    _raw = json.load(_f)
KEYWORD_CARD_IMAGES = {k: v for k, v in _raw.items() if not k.startswith('_')}
del _raw, _f



# ── Unit image map (slug -> CDN filename, for unit DB builder) ───────────────
UNIT_IMAGE_MAP = {}


# ── Bundled keyword definitions (offline fallback) ───────────────────────────
BUNDLED_KEYWORDS = {
